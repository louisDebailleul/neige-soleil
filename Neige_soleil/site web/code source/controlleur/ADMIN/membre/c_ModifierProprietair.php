...
$bd = App::getDatabase();
$id = $_GET['id'];
$client = $bd->query("select * from proprietair where id = $id ")->fetch();
...
if(isset($_POST['modifier'])){
      ...
      if(empty($_SESSION['erreur'])){
           ...
           // comfirme email
            if($email == $emailConfirme ){
                ...
                if($client->email != $email ){
                    $verif = $bd->query(" select * from proprietair where email = '$email' ")->fetchall();
                    $count = count($verif);
                    $_SESSION['test']= $count;
                    if($count == 1){
                ...
                if($passwordConfime != $password ){
                    ...
                    header("Location: ?p=monNeigeEtSoleil&gestion=ModifierProprietair&id=$id");
                    exit();
                 }
                ...
                if(empty($_SESSION['erreur'])){
                   ...
                   $bd->query("update proprietair SET nom = ?, password = md5(?),prenom = ?, email = ? , ville= ? ,adresse = ? , codeP = ? ,tel = ?
                where id = $id ", [
            $nom,
            $password,
            $prenom, 
           ...
           $_SESSION['erreur']  .=  "<span> L'email et la confirmation ne sont pas les mêmes .</span><br>";
            header("Location: ?p=monNeigeEtSoleil&gestion=ModifierProprietairr&id=$id");
           exit();
          }
...
require 'vue/ADMIN/membre/ModifierProprietair.php';
