      ...
      if(empty($_SESSION['erreur'])){
           ...
           // comfirme email
            if($email == $emailConfirme ){
                ...
                $verif = $bd->query("select * from proprietair where email = '".$email."'")->fetch();
                
                if($verif != null){
                $_SESSION['erreur'] .= "<span> L'email est deja utiliser.</span><br>";
                header("Location: ?p=monNeigeEtSoleil&gestion=AjouterProprietair");
                exit();
                }
                ...
                if(empty($_SESSION['erreur'])){
                 ...
                 $bd->query("insert into proprietair (nom , password,prenom, email, ville,adresse, codeP,tel)
...
require 'vue/ADMIN/membre/AjouterProprietair.php';
